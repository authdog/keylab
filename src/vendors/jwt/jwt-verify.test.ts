...
it("verifies token audience", async () => {
    ...
    // wrong audience
    const token2 = await signJwtWithPrivateKey({ aud: c.AUTHDOG_ID_ISSUER }, Algs.HS256, "secret")
    ...
    const token3 = await signJwtWithPrivateKey({ aud: c.AUTHDOG_ID_ISSUER }, Algs.HS256, "secret")
    ...
    const token4 = await signJwtWithPrivateKey({ aud: [c.AUTHDOG_ID_ISSUER] }, Algs.HS256, "secret")
    ...
    const token5 = await signJwtWithPrivateKey({ aud: [c.AUTHDOG_ID_ISSUER] }, Algs.HS256, "secret")
...
it("verifies a token with checkTokenValidness signed with ES512 key - jwk", async () => {
    ...
    fetchMock.mockIf("https://as.example.com/jwks", () => ({
        status: 200,
...
it("verifies a token with checkTokenValidness signed with Ed25519 key - jwk", async () => {
    ...
    const tokenInJwksStoreValidness = await checkTokenValidness(signedPayloadEd25519, {
        jwksUri,
    })
...
it("verifies a token with checkTokenValidness signed with Ed448 key - jwk", async () => {
    ...
    const tokenInJwksStoreValidness = await checkTokenValidness(signedPayloadEd448, {
        jwksUri,
    })
