...
export const buildI18nDocs = async () => {
    ...
    files.map(async (pathMeta: string) => {
        ...
        payload.map((meta) => {
            const INPUT_MDX_PATH = path.resolve(ROOT_WEBSITE_DOC, `docs/${meta.in_mdx}`);
            var data = fs.readFileSync(INPUT_MDX_PATH, 'utf8');
            const payloadMdx = data.toString();
            const indexEndFrontMatter = getNIndex(payloadMdx, FRONT_MATTER_SEP, 2);
            ...
            const OUTPUT_MDX_PATH = path.resolve(ROOT_WEBSITE_DOC, `docs/${meta.out_mdx}`);
