import { readTokenHeaders, getAlgorithmJwt, verifyHSTokenWithSecretString, checkJwtFields, parseJwt } from "./jwt.js";
import { JwtAlgorithmsEnum as Algs, JwtParts, JwtKeyTypes as Kty } from "../../enums.js";
...
it("verifies HS256 token", async () => {
  ...
  const signedToken = await signJwtWithPrivateKey({
    ...
    data: "foobar"
  }, Algs.HS256, SECRET_STRING);
  const isVerified = await verifyHSTokenWithSecretString(signedToken, SECRET_STRING, Algs.HS256);
  expect(isVerified).toBeTruthy();
  const shouldNotBeVerified = await verifyHSTokenWithSecretString(signedToken, "wrong-secret", Algs.HS256);
  expect(shouldNotBeVerified).toBeFalsy();
  const signedTokenAlreadyExpired = await signJwtWithPrivateKey({
    exp: Math.floor(Date.now() / 1000),
    data: "foobar"
  }, Algs.HS256, SECRET_STRING);
  const shouldNotBeVerifiedAsExpired = await verifyHSTokenWithSecretString(signedTokenAlreadyExpired, SECRET_STRING, Algs.HS256);
  expect(shouldNotBeVerifiedAsExpired).toBeFalsy();
  const signedTokenNotExpired = await signJwtWithPrivateKey({
    exp: Math.floor(Date.now() / 1000) + 60 * 60,
    data: "foobar"
  }, Algs.HS256, SECRET_STRING);
  const shouldVerifysignedTokenNotExpired = await verifyHSTokenWithSecretString(signedTokenNotExpired, SECRET_STRING, Algs.HS256);
...
it("verifies token audience", async () => {
  ...
  expect(valid).toBeFalsy(); // wrong audience
  ...
  expect(valid2).toBeFalsy(); // right audience - aud string
  ...
  expect(valid3).toBeTruthy(); // right audience - aud array
  ...
  expect(valid4).toBeTruthy(); // right audience - aud array, but missing `missing-audience`
  ...
  expect(valid5).toBeFalsy(); // issuer
  // wrong issuer
  ...
  expect(valid6).toBeFalsy(); // right issuer
