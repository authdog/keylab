import { getKeyPair } from "./jwt-sign.js";
import { readTokenHeaders } from "./jwt.js";
import * as c from "../../constants.js";
import { JwtAlgorithmsEnum as Algs } from "../../enums.js";
import { parseJwt, signJwtWithPrivateKey } from "./index.js";
import { strToUint8Array, uint8ArrayToStr } from "./utils.js";
...
it("jwt sign with payload fields - RS256", async () => {
  ...
  const signedPayloadRs256 = await signJwtWithPrivateKey({
    ...
    iss: "issuer:12345",
    aud: ["aud:12345"],
    scp: [["a", "b:c", "d"].map(el => el.permission?.name)].join(" ")
  }, Algs.RS256, keyPairRS256.privateKey);
  const {
    iss,
    aud,
    sub,
    aid
  } = parseJwt(signedPayloadRs256);
...
it("jwt sign with payload fields - RS384", async () => {
  ...
  const signedPayloadRs384 = await signJwtWithPrivateKey({
    ...
    aud: ["aud:12345"],
    scp: [["a", "b:c", "d"].map(el => el.permission?.name)].join(" ")
  }, Algs.RS384, keyPairRS384.privateKey);
  const {
    iss,
    aud,
    sub,
    aid
  } = parseJwt(signedPayloadRs384);
  expect(iss).toEqual("issuer:12345");
  expect(aud).toEqual(["aud:12345"]);
  expect(sub).toEqual("sub:12345");
  expect(aid).toEqual("12345");
...
it("jwt sign with payload fields - RS512", async () => {
  ...
  const signedPayloadRs512 = await signJwtWithPrivateKey({
    ...
    aud: ["aud:12345"],
    scp: [["a", "b:c", "d"].map(el => el.permission?.name)].join(" ")
  }, Algs.RS512, keyPairRS512.privateKey);
  const {
    iss,
    aud,
    sub,
    aid
  } = parseJwt(signedPayloadRs512);
  expect(iss).toEqual("issuer:12345");
  expect(aud).toEqual(["aud:12345"]);
  expect(sub).toEqual("sub:12345");
  expect(aid).toEqual("12345");
...
it("jwt sign with payload fields - ES256", async () => {
  ...
  const signedPayloadEs256 = await signJwtWithPrivateKey({
    ...
    aud: ["aud:12345"],
    scp: [["a", "b:c", "d"].map(el => el.permission?.name)].join(" ")
  }, Algs.ES256, keyPairES256.privateKey);
  const {
    iss,
    aud,
    sub,
    aid
  } = parseJwt(signedPayloadEs256);
  expect(iss).toEqual("issuer:12345");
  expect(aud).toEqual(["aud:12345"]);
  expect(sub).toEqual("sub:12345");
  expect(aid).toEqual("12345");
...
it("jwt sign with payload fields - EDDSA", async () => {
  ...
  const signedPayloadEdDSA = await signJwtWithPrivateKey({
    ...
    aud: ["aud:12345"],
    scp: [["a", "b:c", "d"].map(el => el.permission?.name)].join(" ")
  }, Algs.EdDSA, keyPairEDDSA.privateKey);
  const {
    iss,
    aud,
    sub,
    aid
  } = parseJwt(signedPayloadEdDSA);
  expect(iss).toEqual("issuer:12345");
  expect(aud).toEqual(["aud:12345"]);
  expect(sub).toEqual("sub:12345");
  expect(aid).toEqual("12345");
...
it("it converts string to uint8 and vice versa", async () => {
  ...
  const buffer = strToUint8Array(superSecret);
  const debufferedString = uint8ArrayToStr(buffer);
...
it("signs payload with pkcs8 private key", async () => {
  ...
  expect(signedPayloadRs256).toBeTruthy(); // RS384
  ...
  expect(signedPayloadRs384).toBeTruthy(); // RS512
  ...
  expect(signedPayloadRs512).toBeTruthy(); // ES256
  ...
  expect(signedPayloadEs256).toBeTruthy(); // ES384
  ...
  const signedPayloadEs384 = await signJwtWithPrivateKey({
    urn: "urn:test:test"
  }, Algs.ES384, keyPairES384.privateKey);
  expect(signedPayloadEs384).toBeTruthy(); // ES512
  ...
  const signedPayloadEs512 = await signJwtWithPrivateKey({
    urn: "urn:test:test"
  }, Algs.ES512, keyPairES512.privateKey);
...
it("signs payload with pkcs8 private key - RSA-PSS", async () => {
  ...
  const signedPayloadPs256 = await signJwtWithPrivateKey({
    urn: "urn:test:test"
  }, Algs.PS256, keyPairPS256.privateKey);
  expect(signedPayloadPs256).toBeTruthy(); // PS384
  ...
  const signedPayloadPs384 = await signJwtWithPrivateKey({
    urn: "urn:test:test"
  }, Algs.PS384, keyPairPS384.privateKey);
  expect(signedPayloadPs384).toBeTruthy(); // PS512
  ...
  const signedPayloadPs512 = await signJwtWithPrivateKey({
    urn: "urn:test:test"
  }, Algs.PS512, keyPairPS512.privateKey);
...
it("signs payload with pkcs8 private key - ES256k", async () => {
  ...
  const signedPayloadEs256k = await signJwtWithPrivateKey({
    urn: "urn:test:test"
  }, Algs.ES256K, keyPairES256k.privateKey, {
    kid: keyPairES256k?.kid
  });
  expect(signedPayloadEs256k).toBeTruthy();
});
it("signs payload with pkcs8 private key - unsupported yet", async () => {// works but not signin
