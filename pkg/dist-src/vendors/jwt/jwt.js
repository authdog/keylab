...
import { atob } from "../ponyfills/ponyfills.js";
import * as c from "../../constants.js";
import * as enums from "../../enums.js";
import { msg, throwJwtError } from "../../errors/index.js";
import { verifyRSAToken } from "../jwks/index.js";
import { signJwtWithPrivateKey } from "./jwt-sign.js";
/**
 *
 * @param token
...
export const readTokenHeaders = token => {
  let decoded;
...
export const getAlgorithmJwt = token => {
  let algorithm;
...
export const parseJwt = (token, part = enums.JwtParts.PAYLOAD) => {
  let indexPart = -1;
  ...
  var jsonPayload = decodeURIComponent(atob(base64).split(c.EMPTY_STRING).map(c => {
    return "%" + ("00" + c.charCodeAt(0).toString(16)).slice(-2);
  }).join(c.EMPTY_STRING));
  return JSON.parse(jsonPayload);
};
export const checkTokenValidness = async (token, {
  secret,
  jwksUri,
  verifySsl = true,
  adhoc,
...
export const verifyHSTokenWithSecretString = async (token, secret, algorithm = enums.JwtAlgorithmsEnum.HS256) => {
  let isValid = false;
  ...
  if (isValid) {
    const {
      exp
    } = parseJwt(token, enums.JwtParts.PAYLOAD);
...
export const checkJwtFields = (token, {
  requiredAudiences = [],
  requiredIssuer = null,
  requiredScopes = []
}) => {
  ...
  try {
    const parsedToken = parseJwt(token); // audience

    if (parsedToken?.aud && typeof parsedToken?.aud === "string" && requiredAudiences.length > 0) {
      validFields = parsedToken?.aud === requiredAudiences[0];
    } else if (parsedToken?.aud && Array.isArray(parsedToken?.aud) && requiredAudiences.length > 0) {
      requiredAudiences.map(el => {
...
export const createSignedJwt = async (payload, {
  ...
  const algEnums = enums.JwtAlgorithmsEnum;
  let token; // TODO: reflect all fields standard JWT
  ...
  switch (algorithm) {
    ...
    case algEnums.ES256K:
      if (signinOptions?.pemPrivateKey) {
        token = await signJwtWithPrivateKey(jwtClaims, algorithm, signinOptions.pemPrivateKey);
      } else if (signinOptions?.jwkPrivateKey) {
        token = await signJwtWithPrivateKey(jwtClaims, algorithm, signinOptions.jwkPrivateKey);
      }
