...
import { throwJwtError } from "../../errors/index.js";
import { JwtAlgorithmsEnum as Algs } from "../../enums.js";
import * as c from "../../constants.js";
import * as jwt from "jsonwebtoken";
import { checkJwtFields, readTokenHeaders } from "../jwt/index.js";
...
const defaultKeyOptions = algorithm => {
...
export const generateKeyFromStore = async (store, keyType, algorithm, exposePrivateFields = false, keySize = 2048) => {
  ...
  switch (algorithm) {
    ...
    case Algs.RS512:
    case Algs.RSAPSS:
      generatedKey = await store.generate(keyType.toUpperCase(), keySize, { ...defaultKeyOptions(algorithm)
      });
      break;
    // EC
    ...
    case Algs.ES384:
    case Algs.ES512:
      generatedKey = await store.generate(keyType.toUpperCase(), c.namedCurves[algorithm.toLowerCase()], { ...defaultKeyOptions(algorithm)
      });
      break;
...
export const generateJwtFromPayload = async ({
  ...
  const token = await jose.JWS.createSign(Object.assign({
    compact,
    jwk,
    fields
  }), jwk).update(payload).final();
  return token;
};
export const generatePrivateJwk = async (keyType, algorithm) => {
  const store = createKeyStore();
  const jsonWebKey = await generateKeyFromStore(store, keyType, algorithm, true);
  return jsonWebKey;
...
export const makePublicKey = privateKey => {
  const publicKey = {
    kty: privateKey.kty,
    kid: privateKey.kid,
    use: privateKey.sig,
...
export const keyExistsInSet = (keyId, jwks) => {
  const exists = jwks.find(jwk => jwk.kid === keyId);
...
export const getKeyFromSet = (keyId, jwks) => {
...
export const verifyRSAToken = async (token, {
  jwksUri,
  verifySsl = false,
  requiredAudiences = [],
  requiredIssuer = null,
  ...
  const jwksResource = !!adhoc ? adhoc : await fetchJwksWithUri({
    jwksUri,
    verifySsl
  });
  let verified = false;
