import { createKeyStore, generateKeyFromStore, keyExistsInSet } from "../index.js";
import { JwtAlgorithmsEnum as Algs, JwtKeyTypes as Kty } from "../../enums.js";
import { default as nock } from "nock";
import { makePublicKey, verifyRSAToken, generateJwtFromPayload } from "./jwks.js";
...
it("check if key exists in set", () => {
  ...
  const exists = keyExistsInSet("THxOr1j3MA7jcXKNCJvrqHysckPAcgma-vm0w0HPP00", jwks);
  expect(exists).toBeTruthy();
  const shouldNotExist = keyExistsInSet("THxOr1j3MA7jcXKNCJvrqHysckPAcgma-vm0w0HPP00", []);
...
it("verifies correctly token with public uri", async () => {
  ...
  const keyGenerated = await generateKeyFromStore(store, Kty["RSA"].toUpperCase(), Algs.RS256, exposeJwkPrivateFields);
  const regExpPathAppJwks = new RegExp(`api\/${c.AUTHDOG_JWKS_API_ID}\/${tenantUuid2}\/${applicationUuid2}\/.well-known\/jwks.json*`);
  const keys = [makePublicKey(keyGenerated)];
  const scopeNock = nock(AUTHDOG_API_ROOT).persist().get(regExpPathAppJwks).reply(200, {
    keys
  });
  const payload = {
...
it("generates jwk with generateKeyFromStore", async () => {
  const store = createKeyStore();
  const keyRsa256 = await generateKeyFromStore(store, Kty["RSA"].toUpperCase(), Algs.RS256, true);
  expect(keyRsa256).toBeTruthy();
  const keyRsa384 = await generateKeyFromStore(store, Kty.RSA, Algs.RS384, true);
  expect(keyRsa384).toBeTruthy();
  const keyRsa512 = await generateKeyFromStore(store, Kty.RSA, Algs.RS512, true);
  expect(keyRsa512).toBeTruthy();
  const keyEs256 = await generateKeyFromStore(store, Kty.EC, Algs.ES256, true); // console.log(keyEs256);

  expect(keyEs256).toBeTruthy();
  const keyEs384 = await generateKeyFromStore(store, Kty.EC, Algs.ES384, true);
  expect(keyEs384).toBeTruthy();
  const keyEs512 = await generateKeyFromStore(store, Kty.EC, Algs.ES512, true);
  expect(keyEs512).toBeTruthy(); // PS256 / PS384 / PS512
  // also valid for PS384 and PS512 as RSA-PSS is used for all of them

  const keyPs256 = await generateKeyFromStore(store, Kty.RSA, Algs.RSAPSS, true);
